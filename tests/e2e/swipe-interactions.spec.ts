...
test.describe('Swipe Interactions', () => {
	...
	async function startRanking(page: any, items = ['Item A', 'Item B', 'Item C']) {
	...
	test('should show swipe instructions', async ({ page }) => {
		...
		await expect(page.getByText(/swipe right to choose|swipe or tap to choose/i)).toBeVisible({ timeout: 5000 });
	...
	test('should handle tap selection', async ({ page }) => {
		...
		await expect(page.getByText(/ranking complete|confirm choice/i)).toBeVisible({ timeout: 10000 });
	});
	...
	test('should maintain interaction state', async ({ page }) => {
		...
		await expect(page.getByText(/test fruits|which do you prefer|ranking complete/i)).toBeVisible({ timeout: 10000 });
	});
